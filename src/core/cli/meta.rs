...
use super::{
    comm_data::CommData,
    debug::debug_mode,
    graphql_client::{self, linera_service, linera_service_kill},
    lurk_data::LurkData,
    microchain::{
        postprocess, read_data, write_data, CallableData, ChainState, PostprocessData, Request,
        Response,
    },
    ...
    const LOAD: Self = Self {
        ...
        run: |repl, args, path| {
            Box::pin(async move {
                ...
                repl.load_file(&path.join(file_name), false).await?;
                Ok(*repl.zstore.t())
            })
        },
    };
...
impl<C1: Chipset<F>, C2: Chipset<F>> MetaCmd<F, C1, C2> {
    ...
    const VERIFY: Self = Self {
        ...
        run: |repl, args, _dir| {
            Box::pin(async move {
                ...
                let has_same_verifier_version =
                    cached_proof.crypto_proof.has_same_verifier_version();
    ...
    const VERIFY_PROTOCOL: Self = Self {
        ...
        run: |repl, args, _dir| {
            Box::pin(async move {
                ...
                let ProtocolProof { crypto_proof, args } =
                    bincode::deserialize(&protocol_proof_bytes)?;
    ...
    const LINERA_START: Self = Self {
        ...
        run: |repl, args, _dir| {
            Box::pin(async move {
                if repl.linera {
                    let [&chain_id, &contract, &service] = repl.take(args)?;
                    let (chain_id, _) = repl.reduce_aux(&chain_id)?;
                    let (contract, _) = repl.reduce_aux(&contract)?;
                    let (service, _) = repl.reduce_aux(&service)?;
                    if chain_id.tag != Tag::Str
                        || contract.tag != Tag::Str
                        || service.tag != Tag::Str
                    {
                        bail!("Both contract and service must be strings");
                    }

                    let chain_id = repl.zstore.fetch_string(&chain_id);
                    let contract = repl.zstore.fetch_string(&contract);
    ...
    fn linera_microchain_start<'a>(
        ...
        Box::pin(async move {
            let [&port, &chain, &app, &owner, &state_expr] = repl.take(args)?;
            let (port, _) = repl.reduce_aux(&port)?;
            let (chain_zptr, _) = repl.reduce_aux(&chain)?;
            let (app, _) = repl.reduce_aux(&app)?;
            let (owner, _) = repl.reduce_aux(&owner)?;
            if port.tag != Tag::Str
                || chain_zptr.tag != Tag::Str
                || app.tag != Tag::Str
                || owner.tag != Tag::Str
            {
                bail!("All args except state must be strings");
            }
            ...
            let chain = repl.zstore.fetch_string(&chain_zptr);
            let app = repl.zstore.fetch_string(&app);
    ...
    const MICROCHAIN_VERIFY: Self = Self {
        ...
        run: |repl, args, _dir| {
            Box::pin(async move {
                let [&addr_expr, &id_expr, &initial_state_expr, &final_state_expr] =
                    repl.take(args)?;
                let (addr, _) = repl.reduce_aux(&addr_expr)?;
                if addr.tag != Tag::Str {
                    bail!("Address must be a string");
    ...
    const LOAD_OCAML: Self = Self {
        ...
        returns: "t",
        run: |repl, args, path| Box::pin(async move {
            let [file_name_zptr] = repl.take(args)?;
...
mod test {
    ...
    async fn test_def() {
        ...
        assert!((MetaCmd::DEF.run)(&mut repl, &args, dummy_path())
            .await
            .is_err());
        ...
        let args = repl.zstore.intern_list([foo, a]);
        assert_eq!(
            (MetaCmd::DEF.run)(&mut repl, &args, dummy_path())
                .await
                .unwrap(),
            foo
        );
